(function(App) {
    ...
    var _this;
    App.View.BookDetail = Marionette.View.extend({
      template: '#book-detail-tpl',
      ...
      initialize: function() {
        ...
        App.vent.on('shortcuts:books', _this.initKeyboardShortcuts);
      ...
      onAttach: function() {
        win.info('Show book details (' + this.model.get('imdb_id') + ')');
  
        App.BookDetailView = this;
      ...
      getMetaData: function () {
        ...
        (!this.model.get('synopsis') || (Settings.translateSynopsis && Settings.language !== 'en')) && book && book.overview ? this.model.set('synopsis', book.overview) : null;
        (!this.model.get('rating') || this.model.get('rating') === '0' || this.model.get('rating') === '0.0') && book && book.vote_average ? this.model.set('rating', book.vote_average) : null;
        (!this.model.get('runtime') || this.model.get('runtime') === '0') && book && book.runtime ? this.model.set('runtime', book.runtime) : null;
        !this.model.get('trailer') && book && book.videos && book.videos.results && book.videos.results[0] ? this.model.set('trailer', 'http://www.youtube.com/watch?v=' + book.videos.results[0].key) : null;
        (!this.model.get('poster') || this.model.get('poster') === 'images/posterholder.png') && book && book.poster_path ? this.model.set('poster', 'http://image.tmdb.org/t/p/w500' + book.poster_path) : null;
        (!this.model.get('backdrop') || this.model.get('backdrop') === 'images/posterholder.png') && book && book.backdrop_path ? this.model.set('backdrop', 'http://image.tmdb.org/t/p/w500' + book.backdrop_path) : ((!this.model.get('backdrop') || this.model.get('backdrop') === 'images/posterholder.png') && book && book.poster_path ? this.model.set('backdrop', 'http://image.tmdb.org/t/p/w500' + book.poster_path) : null);
        !this.model.get('tmdb_id') && book && book.id ? this.model.set('tmdb_id', book.id) : null;
        if (book && book.credits && book.credits.cast && book.credits.crew && (book.credits.cast[0] || book.credits.crew[0])) {
          curSynopsis.old = this.model.get('synopsis');
          curSynopsis.crew = book.credits.crew.filter(function (el) {return el.job === 'Director';}).map(function (el) {return '<span>' + el.job + '&nbsp;-&nbsp;</span><span' + (el.profile_path ? ` data-toggle="tooltip" title="<img src='https://image.tmdb.org/t/p/w154${el.profile_path}' class='toolcimg'/>" ` : ' ') + `class="cname" onclick="nw.Shell.openExternal('https://yts.mx/browse-books/${el.name.replace(/\'/g, ' ').replace(/\ /g, '+')}')" oncontextmenu="nw.Shell.openExternal('https://www.imdb.com/find?s=nm&q=${el.name.replace(/\'/g, ' ').replace(/\ /g, '+')}')">${el.name.replace(/\ /g, '&nbsp;')}</span>`;}).join('&nbsp;&nbsp; ') + '<p class="sline">&nbsp;</p>';
          curSynopsis.allcast = book.credits.cast.map(function (el) {return '<span' + (el.profile_path ? ` data-toggle="tooltip" title="<img src='https://image.tmdb.org/t/p/w154${el.profile_path}' class='toolcimg'/>" ` : ' ') + `class="cname" onclick="nw.Shell.openExternal('https://yts.mx/browse-books/${el.name.replace(/\'/g, ' ').replace(/\ /g, '+')}')" oncontextmenu="nw.Shell.openExternal('https://www.imdb.com/find?s=nm&q=${el.name.replace(/\'/g, ' ').replace(/\ /g, '+')}')">${el.name.replace(/\ /g, '&nbsp;')}</span><span>&nbsp;-&nbsp;${el.character.replace(/\ /g, '&nbsp;')}</span>`;}).join('&nbsp;&nbsp; ') + '<p>&nbsp;</p>';
          curSynopsis.cast = book.credits.cast.slice(0,10).map(function (el) {return '<span' + (el.profile_path ? ` data-toggle="tooltip" title="<img src='https://image.tmdb.org/t/p/w154${el.profile_path}' class='toolcimg'/>" ` : ' ') + `class="cname" onclick="nw.Shell.openExternal('https://yts.mx/browse-movies/${el.name.replace(/\'/g, ' ').replace(/\ /g, '+')}')" oncontextmenu="nw.Shell.openExternal('https://www.imdb.com/find?s=nm&q=${el.name.replace(/\'/g, ' ').replace(/\ /g, '+')}')">${el.name.replace(/\ /g, '&nbsp;')}</span><span>&nbsp;-&nbsp;${el.character.replace(/\ /g, '&nbsp;')}</span>`;}).join('&nbsp;&nbsp; ') + (book.credits.cast.length > 10 ? '&nbsp;&nbsp;&nbsp;<span class="showall-cast">more...</span>' : '') + '<p>&nbsp;</p>';
        }
      },
      ...
      closeDetails: function() {
        App.vent.trigger('book:closeDetail');
      },
